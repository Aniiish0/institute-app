...
const GenericForm = <T extends object>({ initialValues, fields, onSubmit }: GenericFormProps<T>) => {
  ...
  const formValidator: React.MutableRefObject<{ [key: string]: ValidatorType }> = useRef(
    getFieldValidators(fields)
  );
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    for (const key in formState) {
      const validator = formValidator.current[key as keyof typeof formValidator.current];
      if (typeof validator === 'function' && typeof formState[key] === 'string' && !validator(formState[key])) {
        e.stopPropagation();
        ...
        <div key={field.label} className='pb-3 rounded-0'>
          {field.type === 'textarea' ? (
            ...
            <TextInput
              ...
              placeholder={field.placeholder}
              required={field.required}
              validator={field.validateRegExp !== undefined ? ((value: string) => value === '' || field.validateRegExp?.test(value) || false) : undefined}
              validationError={field.validationError}
            />
          )}
        </div>
